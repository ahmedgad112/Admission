...
const modalConfig = computed<TwoFactorConfigContent>(() => {
    if (props.twoFactorEnabled) {
        return {
            title: trans('settings.security.2fa.modal_enabled_title'),
            description: trans('settings.security.2fa.modal_enabled_description'),
            buttonText: trans('settings.security.2fa.close'),
        };
    }

    if (showVerificationStep.value) {
        return {
            title: trans('settings.security.2fa.verify_title'),
            description: trans('settings.security.2fa.verify_description'),
            buttonText: trans('settings.security.2fa.continue'),
        };
    }

    return {
        title: trans('settings.security.2fa.modal_enable_title'),
        description: trans('settings.security.2fa.modal_enable_description'),
        buttonText: trans('settings.security.2fa.continue'),
    };
});
            ...
            >
                <template v-if="!showVerificationStep">
                    <AlertError
                        v-if="errors?.length"
                        :errors="errors"
                        :title="trans('settings.security.2fa.error_title')"
                    />
                    <template v-else>
                        <div
                        ...
                        >
                            ...
                            <span class="bg-card relative px-2 py-1">{{
                                trans('settings.security.2fa.manual')
                            }}</span>
                        </div>
                ...
                <template v-else>
                    <Form
                        ...
                        v-slot="{ errors: confirmErrors, processing }"
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <InputError :message="confirmErrors?.code" />
                            </div>
                            ...
                            <div class="flex w-full items-center space-x-5">
                                ...
                                >
                                    {{ trans('settings.security.2fa.back') }}
                                </Button>
                                <Button
                                    type="submit"
                                    class="w-auto flex-1"
                                ...
                                >
                                    {{ trans('settings.security.2fa.confirm') }}
                                </Button>
                            </div>
                        </div>
                    </Form>
