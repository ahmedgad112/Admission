...
const form = useForm({
    ...
    branch_id: props.member?.branch_id ?? props.defaultBranchId ?? props.branches[0]?.id ?? '',
...
const branchDepartments = computed(() =>
    props.departments.filter((department) => Number(department.branch_id) === Number(form.branch_id)),
);
...
watch(
    () => form.role,
    () => {
        form.permissions = form.permissions.filter((permission) => !roleDefaults.value.includes(permission));
    },
);
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div class="space-y-2">
                    <Label for="password">
                        ...
                        <span v-if="member" class="font-normal text-muted-foreground"> {{ trans('staff.keep_password') }}</span>
                    </Label>
                ...
                <div class="space-y-2">
                    ...
                    <select id="status" v-model="form.status" class="field-control">
                ...
                <div class="space-y-2">
                    ...
                    <select id="department_id" v-model="form.department_id" class="field-control">
                ...
                <div class="space-y-2">
                    <div class="flex items-center justify-between gap-2">
                        <Label for="shift_id">{{ trans('common.shift') }}</Label>
                        <Link href="/shifts" class="text-xs text-primary">{{ trans('shifts.manage') }}</Link>
                    </div>
                    <select id="shift_id" v-model="form.shift_id" class="field-control">
                        <option value="">{{ trans('common.none') }}</option>
                ...
                <div class="space-y-2">
                    <Label for="leave_days">{{ trans('staff.leave_days') }}</Label>
                    <Input id="leave_days" v-model.number="form.leave_days" type="number" min="0" max="365" />
                    <p class="text-xs text-muted-foreground">{{ trans('staff.leave_days_help') }}</p>
                    <p v-if="form.errors.leave_days" class="text-sm text-destructive">
                ...
                <div class="space-y-3 md:col-span-2">
                    <div class="space-y-1">
                        ...
                        <p class="text-xs text-muted-foreground">{{ trans('staff.permissions_help') }}</p>
                    </div>
                    <div class="grid gap-3 md:grid-cols-2">
                        <label
                            ...
                            :class="{ 'opacity-70': isPermissionLocked(permission.value) }"
                        >
                            <Checkbox
                                class="mt-0.5"
