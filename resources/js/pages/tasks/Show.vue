...
function transition(status: string): void {
    router.post(`/tasks/${props.task.id}/transition`, { status }, { preserveScroll: true });
}
    ...
    <div class="page-shell">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <Button v-if="canUpdate" variant="outline" class="rounded-full" as-child>
                    <Link :href="`/tasks/${task.id}/edit`">{{ trans('common.edit') }}</Link>
                </Button>
                <Button v-if="canDelete" variant="destructive" class="rounded-full" @click="destroy">
                    {{ trans('common.delete') }}
                </Button>
            </div>
        </div>
            ...
            <div class="space-y-4">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            {{
                                task.due_date
                                    ? trans('tasks.due', { date: task.due_date })
                    ...
                    <CardContent class="space-y-4">
                        <p class="whitespace-pre-wrap text-sm">
                            {{ task.description || trans('tasks.no_description') }}
                        </p>
                        <div v-if="canUpdate" class="flex flex-wrap gap-2">
                            <Button size="sm" variant="outline" class="rounded-full" @click="transition('todo')">{{ trans('status.todo') }}</Button>
                            <Button size="sm" variant="outline" class="rounded-full" @click="transition('in_progress')">{{ trans('status.in_progress') }}</Button>
                            <Button size="sm" variant="outline" class="rounded-full" @click="transition('review')">{{ trans('status.review') }}</Button>
                            <Button size="sm" class="rounded-full" @click="transition('completed')">{{ trans('status.completed') }}</Button>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent class="space-y-4">
                        ...
                        >
                            <div class="mb-1 text-sm font-medium">{{ comment.user?.name }}</div>
                        ...
                        <div class="space-y-2">
                            <textarea
                                ...
                                :placeholder="trans('tasks.comment_placeholder')"
                            />
                            <Button size="sm" :disabled="commentForm.processing" @click="addComment">
            ...
            <div class="space-y-4">
                <Card>
                    ...
                    <CardContent class="space-y-3">
                        ...
                        <Input
                            type="file"
                            @change="attachmentForm.file = (($event.target as HTMLInputElement).files?.[0] ?? null)"
                        />
                        <Button size="sm" variant="outline" :disabled="attachmentForm.processing" @click="upload">
                            {{ trans('tasks.upload') }}
                        </Button>
                    </CardContent>
                </Card>
