...
<template>
    ...
    <div class="page-shell space-y-6">
        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            >
                <div
                    class="flex size-10 items-center justify-center rounded-xl bg-primary/10 text-primary transition-colors group-hover:bg-primary group-hover:text-primary-foreground"
                >
                    <ScanLine class="size-5" />
                </div>
                <div class="min-w-0">
                    <p
                        class="text-xs font-semibold text-foreground group-hover:text-primary"
                    >
            ...
            >
                <div
                    class="flex size-10 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-600 transition-colors group-hover:bg-emerald-600 group-hover:text-white dark:text-emerald-400"
                >
                    <CalendarDays class="size-5" />
                </div>
                <div class="min-w-0">
                    <p
                        class="text-xs font-semibold text-foreground group-hover:text-emerald-600 dark:group-hover:text-emerald-400"
                    >
                        {{ trans('dashboard.create_roster') }}
                    </p>
                    <p class="truncate text-[11px] text-muted-foreground">
                        {{ trans('dashboard.roster_hint') }}
                    </p>
                </div>
            </Link>
            ...
            >
                <div
                    class="flex size-10 items-center justify-center rounded-xl bg-amber-500/10 text-amber-600 transition-colors group-hover:bg-amber-600 group-hover:text-white dark:text-amber-400"
                >
                    <CalendarOff class="size-5" />
                </div>
                <div class="min-w-0">
                    <p
                        class="text-xs font-semibold text-foreground group-hover:text-amber-600 dark:group-hover:text-amber-400"
                    >
            ...
            >
                <div
                    class="flex size-10 items-center justify-center rounded-xl bg-sky-500/10 text-sky-600 transition-colors group-hover:bg-sky-600 group-hover:text-white dark:text-sky-400"
                >
                    <ClipboardList class="size-5" />
                </div>
                <div class="min-w-0">
                    <p
                        class="text-xs font-semibold text-foreground group-hover:text-sky-600 dark:group-hover:text-sky-400"
                    >
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <Card
                class="metric-card-hover relative overflow-hidden border-0 bg-gradient-to-br from-primary via-[hsl(174_62%_28%)] to-[hsl(192_48%_20%)] text-primary-foreground shadow-lg"
            >
                <div
                    class="pointer-events-none absolute -end-6 -bottom-6 size-32 rounded-full bg-white/5 blur-2xl"
                ></div>
                <CardHeader class="pb-2">
                    <div class="flex items-center justify-between">
                        <p
                            class="text-xs font-medium tracking-wider text-white/80 uppercase"
                        >
                            {{ trans('dashboard.attendance_rate') }}
                        </p>
                        <div
                            class="flex size-9 items-center justify-center rounded-xl bg-white/10 backdrop-blur-md"
                        >
                            <CheckCircle2 class="size-5 text-white" />
                        </div>
                    </div>
                    <CardTitle class="mt-1 text-4xl font-extrabold text-white">
                ...
                <CardContent class="space-y-2 text-sm text-white/80">
                    <p class="text-xs">
                        {{
                            trans('dashboard.present_of_alt', {
                                present: metrics.present_today,
                                total: metrics.headcount,
                            })
                        }}
                    </p>
                    <!-- Progress Bar -->
                    ...
                    >
                        <div
                            ...
                            :style="{
                                width: `${Math.min(metrics.attendance_rate, 100)}%`,
                            }"
                        ></div>
                    </div>
                </CardContent>
            ...
            <Card class="metric-card-hover border-border/80 shadow-xs">
                <CardHeader class="pb-2">
                    ...
                    <CardTitle class="mt-1 text-4xl font-extrabold">
                        {{ metrics.late_today }}
                    </CardTitle>
                </CardHeader>
                <CardContent class="text-xs text-muted-foreground">
                    ...
                    >
                        ...
                        {{
                            trans('dashboard.late_month', {
                                count: metrics.late_this_month,
                            })
                        }}
                    </span>
                </CardContent>
            ...
            <Card class="metric-card-hover border-border/80 shadow-xs">
                <CardHeader class="pb-2">
                    <div class="flex items-center justify-between">
                        <p
                            class="text-xs font-medium tracking-wider text-muted-foreground uppercase"
                        >
                            {{ trans('dashboard.task_completion') }}
                        </p>
                        <div
                ...
                <CardContent class="space-y-2 text-xs text-muted-foreground">
                    <p>
                        {{
                            trans('dashboard.tasks_completed', {
                    ...
                    >
                        <div
                            ...
                            :style="{
                                width: `${Math.min(metrics.task_completion_rate, 100)}%`,
                            }"
                        ></div>
                    </div>
                </CardContent>
        ...
        <Card class="overflow-hidden border-border/80 shadow-xs">
            <CardHeader
                class="flex flex-col gap-3 border-b border-border/60 bg-muted/20 pb-4 sm:flex-row sm:items-center sm:justify-between"
            >
                <div>
                    <div class="flex items-center gap-2">
                        <CardTitle class="text-lg font-bold">{{
                            trans('dashboard.floor')
                        }}</CardTitle>
                        <span
            ...
            <CardContent class="p-0">
                ...
                >
                    <div
                        class="mb-3 flex size-14 items-center justify-center rounded-full bg-muted/60 text-muted-foreground"
                    >
                        <Users class="size-6" />
                    </div>
                    ...
                    <p class="mt-1 max-w-sm text-xs text-muted-foreground">
                        يمكن للموظفين استخدام زر "امسح الآن" أو شاشة الحضور
                ...
                >
                    ...
                    >
                        <Avatar
                            class="size-9 border border-border/60 shadow-xs"
                        >
                            <AvatarImage
                        ...
                        <div class="min-w-0 flex-1 space-y-2">
                            ...
                            >
                                <span
                                    class="inline-flex items-center gap-1.5 rounded-md bg-muted/50 px-2.5 py-1"
                                >
